...
function AttemptPurchase(type, upgradeLevel)
    ...
    TriggerServerEvent("qb-customs:attemptPurchase", type, upgradeLevel, vehiclePrice)
    ...
    while true do
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            for k, v in pairs(bennyGarages) do
                ...
                if nearDefault and ((v.useJob and isAuthorized(job.name, k)) or not v.useJob) then
...
function isAuthorized(job, location)
    if job == bennyGarages[location].job then
        return true
    end
    return false
end