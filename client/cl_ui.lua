...
function InitiateMenus(isMotorcycle, vehicleHealth)
    ...
    --#[Main Menu]#--
    createMenu("mainMenu", "Welcome to THRIVE Customs", "Choose a Category")
    ...
    for k, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
            if v.id == 11 or v.id == 12 or v.id == 13 or v.id == 15 or v.id == 16 then --Performance Upgrades
                ...
                for m, n in pairs(validMods) do
                    ...
                    if maxVehiclePerformanceUpgrades == 0 then
                        populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.performance.prices[tempNum] / 100))
            ...
            elseif v.id == 18 then
                ...
                populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Enable", "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.turbo.price / 100))
                ...
                for m, n in pairs(validMods) do
                    populateMenu(v.category:gsub("%s+", "") .. "Menu", n.id, n.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.cosmetics.price / 100))
    ...
    for k, v in ipairs(vehicleResprayOptions) do
        ...
        for m, n in ipairs(v.colours) do
            populateMenu(v.category .. "Menu", n.id, n.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.respray.price / 100))
        end
    ...
    for k, v in ipairs(vehicleWheelOptions) do
        if v.id == -1 then
            ...
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 0, "Disable", "$0")
            populateMenu(v.category:gsub("%s+", "") .. "Menu", 1, "Enable", "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.customwheels.price / 100))
        ...
        elseif v.id ~= 20 then
            if isMotorcycle then
                if v.id == 6 then --Motorcycle Wheels
                    ...
                    for m, n in pairs(validMods) do
                        populateMenu(v.category .. "Menu", n.id, n.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.wheels.price / 100))
                    end
    ...
    for k, v in ipairs(vehicleTyreSmokeOptions) do
        populateMenu("TyreSmokeMenu", k, v.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.wheelsmoke.price / 100))
    ...
    for k, v in ipairs(vehicleWindowTintOptions) do
        populateMenu("WindowTintMenu", v.id, v.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.windowtint.price / 100))
...
if livCount > 0 then
  ...
  finishPopulatingMenu("VehicleExtrasMenu")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonTypes) do
        ...
        populateMenu(v.name:gsub("%s+", "") .. "Menu", 1, "Enabled", "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.neonside.price / 100))
    ...
    for k, v in ipairs(vehicleNeonOptions.neonColours) do
        populateMenu("NeonColoursMenu", k, vehicleNeonOptions.neonColours[k].name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.neoncolours.price / 100))
    ...
    populateMenu("HeadlightsMenu", 1, "Enable Xenons", "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.headlights.price / 100))
    ...
    for k, v in ipairs(vehicleXenonOptions.xenonColours) do
        populateMenu("XenonColoursMenu", v.id, v.name, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.xenoncolours.price / 100))
    ...
    if state then
            ...
            elseif isMenuActive("NeonsSideMenu") then
                ...
                if currentMenu == "repairMenu" then
                    if AttemptPurchase("repair") then
                        ...
                        updateMenuStatus("Not Enough Toolkit")
                    end
        ...
        elseif isMenuActive("WheelsMenu") then
            if currentWheelCategory ~= 20 and currentWheelCategory ~= -1 then
                ...
                updateItem2Text(currentMenu, currentWheel, "$" .. math.floor(vehiclePrice * vehicleCustomisationPrices.wheels.price / 100))
